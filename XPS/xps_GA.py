from ..nano_indent.helper import *
from ..nano_indent.import_lib import *
from ..nano_indent.ini_parser import *
from ..nano_indent.pathObj import OliverPharr
from .individual import Individual
from ..nano_indent.pathrange import Pathrange_limits
#from .nano_neo_data import NanoIndent_Data
from ..gui.xps_data import xps_data #Should we change names to not be confusing --> xps_data = XPS_data?
from .xps_fit import fit, peak, background #think we may only need to import fit but we'll keep the others for now
...
Comments:
paths and npaths are arrays containing the number of free parameters being optimized for the fit
...
class XPS_GA:
    ...
    def initialize_variable(self):
        ...
        # Paths
        self.npaths = npaths #-----From paths_dict--------
        self.fits = fits
        ...
        # Crosover Parameters
        self.n_bestsam = int(best_sample*self.npops*(0.01)) #--------------It's taking the best 1% of the population to use for future generations?---------
    ...
    def initialize_range(self,i=0,BestIndi=None):
        ...
        self.data_obj = xps_data(self.read_file_txt) #---------------------------------------Call whichever function takes the input data ----------------------------------------------------------

        self.data_obj.pre_processing(limits=(self.data_cutoff[0],self.data_cutoff[1])) #----------------------------------Dont believe we need to process our data at all---------------------------
        #self.x_slice = self.data_obj.get_slice_data()[:,0] #--------Should just be able to have the arrays come straight from file--------
        #self.y_slice = self.data_obj.get_slice_data()[:,1] #---------May need to change it a bit to allow for switch to KE------------
        x_data = xps_data.self.x  #---------Instead just call the data arrays themselves------------ 
        y_data = xps_data.self.y #------Changed all x_slice and y_slice to x_data and y_data--------

        self.pars_range = { #------------These values are declared from ini_parser.py----------
            ''''
            'A_range': A_range, #------------set ranges on parameters so that we don't improbable values i.e. negative Lorentzian------------
            'hf_range': hf_range, #---------Check theory for ranges...FUN-----------
            'm_range': m_range # -----Params we need ranges on: widths, Gauss, Lorentz, BE(center), spin-orbit splitting. (Dont think we need area or amplitude --> too wide a range)-----------
            '''
            'BE_range': BE_range #------dont think we have any other ranges set right now
            #------need sigma, width, and so_split range in the future
         } 

    def create_range(self,value,percentage,dt,prec): #---------Step size for x and y?-------------
        """
        ...
        return range #-------------Do we need this?---------------

    def generateIndividual(self):
        """
        Generate singular individual
        """
        #-------We will need to make/edit our own individual class for XPS--------
        ind = Individual(self.npaths,self.pars_range) #-----------Make list of individuals for first generation given the initial user input values/funcs------------------
        return ind
    ...
    def fitness(self,indObj):
        ...
        #yTotal = np.zeros(len(self.x_slice))-------replacing slices with arrays------
        yTotal = np.zeros(len(self.x))

        for i,paths in enumerate(Individual):

            #y = paths.get_func(self.x_slice)
            y = path.get_func(self.x_data) #------------Is this an array of our fit parameters?--------------
            #-----Does y here represent the y in the formula i.e. the solution to our curve fit or is it just the parameter solutions for the curve fit?---
            yTotal += y #-----getting y array of each fit ------

        for j in range(len(self.x_data)):

            loss = loss + (yTotal[j]*self.x_data[j]**2 - self.y_data[j]* self.x_data[j]**2 )**2 #---------What is happening here?-------
        # if loss == np.nan:
    ...
    def eval_Population(self):
        ...
        populationPerf = {}
        self.nan_counter = 0
        for i,individual in enumerate(self.Populations): #---------Is each individual a population in Individual class?

            temp_score = self.fitness(individual) #---------individual nor Individual? --> what is this variable???--------------
            # Calculate the score, if encounter nan, discard and generate new individual later
            if np.isnan(temp_score):
                self.nan_counter +=1 #------- if the individual is too bad it is thrown out? -----------
            else:
                score.append(temp_score)
                populationPerf[individual] = temp_score #-------Finds the tempoerary best individual fit in the generation------------
        self.sorted_population = sorted(populationPerf.items(), key=operator.itemgetter(1), reverse=False) #------------How is this being sorted?----------
        #-----------itemgetter(1)?????????----------------
        #-----------I am guessing it sorts the population by best to wrost fit or vice versa?--------------------------
        self.currBestFit = self.sorted_population[0] #-------prints out current best fit data -----------------
    ...
    def next_generation(self):
        ...
        # Evaluate Fittness
        score = self.eval_Population() #----------Evaluates the fitness of the previous generation----------
        ...
        #--------What is this comparing the currentBestFit to for the first generation?----------
        ...
        #-----------------------Here is where the next generation is being made------------------------------
        nextBreeders = self.selectFromPopulation() 
        self.logger.info("Number of Breeders: " + str(len(self.parents)))
        self.logger.info("DiffCounter: " + str(self.diffCounter)) #-----DiffCounter keeps track of how many times we mutate-----
        self.logger.info("Diff %: " + str(self.diffCounter / self.genNum)) #-----percent mutated-----
    ...
    def mutatePopulation(self):
        ...
        if self.mut_opt  == 0:
            # Rechenberg mutation
            if self.genNum > 20:
                if self.bestDiff < 0.1: #-----Mutates only after a certain number of generations and then only if the best fit is small (local minima)-----
                    self.diffCounter += 1
                else:
                    self.diffCounter -= 1
    ...
    def selectFromPopulation(self):
        ...
        select_val = np.minimum(self.n_bestsam,len(self.sorted_population)) #------length of how many parents --> depends on the how many are being kept each generation and on the size of the sorted population --------
        self.n_recover = 0
        if len(self.sorted_population) < self.n_bestsam: #-----Dont want the number of parents to be less than the number of best fits kept for each generation-----
            self.n_recover = self.n_bestsam - len(self.sorted_population)
        for i in range(select_val):
            self.parents.append(self.sorted_population[i][0]) #-----Why is sorted_population a 2-d array?-----
    ...
    def export_paths(self,indObj):
        ...
        yTotal = np.zeros(len(self.x_data))
        plt.figure()
        for i,paths in enumerate(Individual):
            y = paths.get_func(self.x_data)
