...
from nano_indent import xps_data
from ..nano_indent.xps_fit import *
from ..nano_indent.xps_individual import *
#from nano_neo_data import NanoIndent_Data

"""
Modified for xps by Evan Restuccia (evan@restuccias.com)
"""
...
class xes_Analysis:
    ...
    def calculate_number_of_params(self,peaks,bkgns):
        """
        ...
        for peakType in peaks:
            #voigt has 4 params
            num += self.get_param_num(peakType)
        
        for bkgn in bkgns:
            #peak shirley/svsc has one param for each peak plusone for the yRight average
            num += self.get_param_num(bkgn)
...
#         print(full_mat)
            bestFit,err = self.construct_bestfit_err_mat(full_mat,plot_err)
#         print(full_mat)
#         print(bestFit)
...
#         best_Fit = np.mean(bestFit,axis=0).reshape(-1,3).round(6)
        ...
        self.individual = self.create_individual_from_bestfit(self.best_Fit_n)
        self.y_model = self.fit_2_arr(self.best_Fit_n)

    def create_individual_from_bestfit(self,best_Fit):
        """
        Creates xps_individual object from a bestfit params to make getting the actual yFit easier
        best_fit (2d array) 
        """
        pars_range = '' #dummy range, since we're going to set the params again later
        ind = Individual(self.bkgns,self.peaks,pars_range)
        peaks = ind.get_peaks()
        bkgns = ind.get_backgrounds()
        for i in range(len(self.peaks)):
            if self.peaks[i] == 'voigt':
...
#         print(S, sig_S, h_max, P, Ac, sig_Ac, H, sig_H, E_r, sig_Er, E, sig_E)
    ...
    def get_params(self): #Error in Analysis --> AttributeError: 'xps_analysis' object has no attribute 'params'
        return self.params
    ...
    #             print(gen_csv.shape)
                ...
                gen_csv_unflatten = gen_csv.reshape((-1,self.number_of_parameters))
                if i == 0:
    ...
    def construct_bestfit_err_mat(self,full_mat,plot=False):
        """
